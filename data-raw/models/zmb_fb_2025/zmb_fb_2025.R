## Code to fit and add zmb_fb_2025 models to database
##
...
dat <- read_facebook_inputs(
  ...
  expected_n = 115L,
  m_env = "NOMAD_ZMB_M_RDS"
)

specs <- facebook_model_specs()

# This is the fitting recipe. In normal package builds we read the archived
# MODEL_OUTPUT objects; set NOMAD_ZMB_REFIT=true to refit from private M.
mods <- if (truthy(Sys.getenv("NOMAD_ZMB_REFIT", "false")) ||
...
model_db <- add_facebook_models_to_db(
  ...
  models = mods,
  model_dir = model_dir,
  plot_codes = facebook_plot_codes("NOMAD_ZMB_CHECK_PLOTS"),
  specs = specs
)
