## Code to fit and add bfa_fb_2025 models to database
##
...
dat <- read_facebook_inputs(
  ...
  expected_n = 45L,
  m_env = "NOMAD_BFA_M_RDS"
)

specs <- facebook_model_specs()

# This is the fitting recipe. In normal package builds we read the archived
# MODEL_OUTPUT objects; set NOMAD_BFA_REFIT=true to refit from private M.
mods <- if (truthy(Sys.getenv("NOMAD_BFA_REFIT", "false")) ||
...
model_db <- add_facebook_models_to_db(
  ...
  models = mods,
  model_dir = model_dir,
  plot_codes = facebook_plot_codes("NOMAD_BFA_CHECK_PLOTS"),
  specs = specs
)
