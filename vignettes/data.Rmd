...
vignette: >
  %\VignetteIndexEntry{nomad databases}
...
The `name` links dataset metadata to fitted models. Four additional fields make
the prediction contract explicit: `prediction_period_days`, `distance_unit`,
`has_fitted_models` and `provenance`. A prediction period is the interval covered
by a trip matrix, not necessarily the dates over which observations were gathered.
Leave unknown periods as NA rather than guessing from those dates.

The new Facebook fits were supplied by JHU. The package records their fitting
recipe in `data-raw/functions/facebook_models.R` and the country scripts. Normal
imports read the supplied fitted objects; refitting is an explicit opt-in and is
not needed to rebuild wrappers. `data-raw/source_metadata.R` verifies distance
scales against supplied geometries and extracts shareable centroids. After model
registration, `data-raw/mobility_db.R` derives availability from actual model IDs.
Run it again after adding fits so the two databases remain consistent.
...
3. Update the period/unit provenance in `mobility_db.R` for the new source, then run it
   to generate the catalogue object before registering the model
...
4. Include shareable inputs and document how restricted inputs are supplied locally.
   Do not commit private trip matrices.
